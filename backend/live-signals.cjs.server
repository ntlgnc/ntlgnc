      ...
      for (const symbol of activeCoins) {
        try {
          ...
          if (signal) {
            ...
            // Signal passed all filters — write as open
            ...
            if (strategy.config && strategy.config.hedging_enabled) {
              ...
              const hedgeMode = strategy.config.hedge_mode || 'exclusive';
              try {
                ...
                const { rows: candidates } = await client.query(
                  `SELECT id, symbol, direction, "entryPrice", "holdBars", strength, "createdAt"
                   ...
                   AND "createdAt" >= NOW() - INTERVAL '${maxGapDays} days'
                   ${pairWhere}
                ...
                if (candidates.length > 0) {
                  ...
                  console.log(`  [HEDGE] Paired ${symbol} ${signal.direction} with ${match.symbol} ${oppositeDir} (pair_id=${pairId.slice(0,8)}...)`);
                }
              } catch (hedgeErr) {
                console.error(`  [HEDGE] Pairing error: ${hedgeErr.message}`);
              }
            }
