/**
 * backfill-daily.cjs — One-time backfill of ALL available daily data
 *
 * Downloads the COMPLETE daily history from Binance for all tracked coins.
 * No row cap — this is a one-time full download. The live-fetch-daily.cjs
 * script handles the 10K rolling cap going forward.
 *
 * Safe to re-run: uses ON CONFLICT DO NOTHING.
 *
 * Usage: node backfill-daily.cjs [--symbol BTCUSDT]
 *
 * BTC has ~3,100 daily candles since Aug 2017. Very fast.
 */
...
async function backfillSymbol(symbol, client, fetchFn) {
  const { rows: [existing] } = await client.query(
    `SELECT MIN(timestamp) as earliest, MAX(timestamp) as latest, COUNT(*)::int as cnt FROM "Candle1d" WHERE symbol = $1`,
    [symbol]
  );
  ...
  if (existing.cnt === 0) {
    ...
    while (true) {
      const candles = await fetchKlines(symbol, '1d', fetchFn, null, endTime, BINANCE_LIMIT);
    ...
    // Fill forward from latest to now
    process.stdout.write(`  ${symbol.padEnd(12)} filling forward (${existing.cnt} existing)...`);
    ...
    console.log(` ${totalInserted} new candles`);
  }
...
async function main() {
  ...
  console.log(`\n═══ Daily Backfill: ${allCoins.length} coins ═══\n`);
  ...
  console.log(`\n═══ Done: ${grandTotal} total daily candles in ${elapsed}s ═══\n`);

  // Print summary
  const { rows } = await client.query(
    `SELECT symbol, COUNT(*)::int as cnt, MIN(timestamp) as earliest, MAX(timestamp) as latest
     FROM "Candle1d" GROUP BY symbol ORDER BY cnt DESC`
  );
  console.log('Symbol       Count     Earliest             Latest');
  console.log('─'.repeat(70));
  for (const r of rows) {
    console.log(`${r.symbol.padEnd(12)} ${String(r.cnt).padStart(7)}   ${r.earliest.toISOString().slice(0,10)}   ${r.latest.toISOString().slice(0,10)}`);
  }
