...
async function runTimeframeLoop(tfKey) {
  ...
  async function tick() {
    ...
    try {
      ...
      let newSignals = 0, closedSignals = 0, pairedSignals = 0, errors = 0;

      // Get open signals to track for closing and duplicate prevention
      ...
      for (const symbol of activeCoins) {
        try {
          ...
          if (signal) {
            ...
            // ── Insert signal as OPEN immediately ──
            const { rows: [newSig] } = await client.query(
              `INSERT INTO "FracmapSignal"
               ("strategyId", symbol, direction, "entryPrice", "detectedPrice", strength, "holdBars",
                "maxCycle", "maxOrder", regime_snapshot, "detectedAt", "enteredAt", tick_id)
               VALUES ($1, $2, $3, $4, $4, $5, $6, $7, $8, $9, now(), now(), $10) RETURNING id`,
              [strategyId, symbol, signal.direction, signal.entryPrice,
               signal.strength, signal.holdBars, signal.maxCycle, signal.maxOrder,
               JSON.stringify(regimeSnap), tickId]
            );

            // ── Natural pair detection: find same-tick opposite signal ──
              ...
              try {
                ...
                const { rows: candidates } = await client.query(
                  `SELECT id, symbol, direction
                   FROM "FracmapSignal"
                   WHERE "strategyId" = $1 AND status = 'open' AND direction = $2
                   AND symbol != $3 AND id != $4 AND pair_id IS NULL
                   AND tick_id = $5
                   ORDER BY strength DESC LIMIT 1`,
                  [strategyId, oppositeDir, symbol, newSig.id, tickId]
                );
                ...
                if (candidates.length > 0) {
                  ...
                  await client.query(
                    `UPDATE "FracmapSignal"
                     SET pair_id = $1, pair_symbol = $2, pair_direction = $3, pair_type = 'natural'
                     WHERE id = $4`,
                    [pairId, match.symbol, match.direction, newSig.id]
                  );
                  await client.query(
                    `UPDATE "FracmapSignal"
                     SET pair_id = $1, pair_symbol = $2, pair_direction = $3, pair_type = 'natural'
                     WHERE id = $4`,
                    [pairId, symbol, signal.direction, match.id]
                  );

                  console.log(`  [NATURAL-PAIR] ${symbol} ${signal.direction} + ${match.symbol} ${oppositeDir} (pair_id=${pairId.slice(0,8)}...)`);
                  pairedSignals++;
                }
              } catch (pairErr) {
                console.error(`  [PAIR] Error: ${pairErr.message}`);
              }
            }
      ...
      if (newSignals > 0 || closedSignals > 0 || pairedSignals > 0) {
