/**
 * Back-propagate two 1m strategies for comparison:
 * 1. "Universal 1m - All Cycles (10-100)" — broad range
 * 2. "Universal 1m - C34 Only" — single peak cycle
 *
 * Both with hedging enabled for paired view on signals page.
 */
...
// 1m strategy params (from live-signals.cjs defaults)
...
function detectEnsembleSignals(bars, allBands) {
  const signals = [];
  let position = null;
  const n = bars.length;
  ...
  for (let i = 1; i < n; i++) {
    ...
    for (const band of allBands) {
      ...
      const buyNear = PARAMS.nearMiss && !buyAtI && (i > 0 && band.lower[i-1] !== null && bars[i-1].low < band.lower[i-1] && bars[i-1].close > band.lower[i-1]);
      if (buyAtI || buyNear) {
        if (PARAMS.spike) { const sH = isLocalMax(band.lower, i, sw); const sN = PARAMS.nearMiss && (isLocalMax(band.lower, i-1, sw) || isLocalMax(band.lower, i+1, sw)); if (!sH && !sN) continue; }
      ...
      const sellNear = PARAMS.nearMiss && !sellAtI && (i > 0 && band.upper[i-1] !== null && bars[i-1].high > band.upper[i-1] && bars[i-1].close < band.upper[i-1]);
      if (sellAtI || sellNear) {
        if (PARAMS.spike) { const sH = isLocalMin(band.upper, i, sw); const sN = PARAMS.nearMiss && (isLocalMin(band.upper, i-1, sw) || isLocalMin(band.upper, i+1, sw)); if (!sH && !sN) continue; }
        sellStrength++; if (band.cycle > maxSellCycle) maxSellCycle = band.cycle; if (band.order > maxSellOrder) maxSellOrder = band.order;
      }
    }
    if (buyStrength >= PARAMS.minStr && buyStrength >= sellStrength) {
      if (PARAMS.priceExt && !isPriceLow(i, Math.round(maxBuyCycle / 2))) {}
      else if (i + 1 < n) {
        const hd = Math.round(maxBuyCycle / PARAMS.holdDiv);
    ...
    } else if (sellStrength >= PARAMS.minStr) {
      if (PARAMS.priceExt && !isPriceHigh(i, Math.round(maxSellCycle / 2))) {}
      else if (i + 1 < n) {
        const hd = Math.round(maxSellCycle / PARAMS.holdDiv);
...
async function createStratAndBackprop(client, name, cycleMin, cycleMax, daysBack) {
  // Create strategy
  const { rows: [strat] } = await client.query(
    `INSERT INTO "FracmapStrategy" (name, type, "barMinutes", "minStr", "minCyc", spike, "nearMiss", "holdDiv", "priceExt", "cycleMin", "cycleMax", active, config, "createdAt", "updatedAt")
     VALUES ($1, 'universal', 1, $2, 0, true, true, 4, true, $3, $4, true, $5, now(), now())
     ON CONFLICT DO NOTHING RETURNING id, name`,
    [name, PARAMS.minStr, cycleMin, cycleMax, JSON.stringify({ hedging_enabled: true, hedge_mode: 'exclusive', max_gap: 5 })]
  );

  if (!strat) {
    // Already exists, get it
    const { rows: [existing] } = await client.query('SELECT id, name FROM "FracmapStrategy" WHERE name = $1 AND active = true', [name]);
    if (!existing) { console.log('  Cannot find/create ' + name); return; }
    console.log('  Using existing: ' + existing.name);
    var stratId = existing.id;
  } else {
  ...
  const { rows: coins } = await client.query(
    `SELECT DISTINCT symbol FROM "Candle1m" WHERE timestamp > NOW() - INTERVAL '${daysBack} days' GROUP BY symbol HAVING COUNT(*) >= 5000`
  );
  console.log('  Coins: ' + coins.length);

  let total = 0, closed = 0, open = 0;
  const lookbackMins = 500; // Extra warmup

  for (const { symbol } of coins) {
    const { rows } = await client.query(
      `SELECT timestamp as time, open, high, low, close FROM "Candle1m" WHERE symbol = $1 AND timestamp > NOW() - INTERVAL '${daysBack + 1} days' ORDER BY timestamp ASC`, [symbol]
    );
    ...
    const signals = detectEnsembleSignals(bars, bands);
    const cutoffIdx = bars.length - (daysBack * 1440);
    ...
    for (const sig of recent) {
      ...
      if (isExpired) {
        ...
        await client.query(
          ...
          [stratId, symbol, sig.type, sig.entryPrice, sig.strength, sig.holdDuration, sig.maxCycle, sig.maxOrder, sig.exitPrice, +(ret.toFixed(4)), exitTime, barTime]
        );
        closed++;
      } else {
        await client.query(
          ...
          [stratId, symbol, sig.type, sig.entryPrice, sig.strength, sig.holdDuration, sig.maxCycle, sig.maxOrder, barTime]
        );
        open++;
      }
      total++;
    }
    if (total > 0 && total % 500 === 0) process.stdout.write('\r  ' + symbol + ' total=' + total);
  }
  ...
  const { rows: [s] } = await client.query(
    `SELECT COUNT(*)::int as total, AVG("returnPct") FILTER (WHERE status='closed') as avg_ret, SUM("returnPct") FILTER (WHERE status='closed') as total_ret, COUNT(*) FILTER (WHERE status='closed' AND "returnPct">0)::int as wins, COUNT(*) FILTER (WHERE status='closed')::int as closed FROM "FracmapSignal" WHERE "strategyId"=$1`, [stratId]
  );
  if (s.closed > 0) {
  ...
  for (let i = 0; i < sigs.length; i++) {
    ...
    for (let j = Math.max(0, i - 200); j < i; j++) {
      ...
      if (gapMs < 0 || gapMs > 5 * 60000) continue; // 5 min gap for 1m
...
(async () => {
  ...
  try {
    console.log('╔══════════════════════════════════════════════════════════════════════╗');
    console.log('║  1m STRATEGY COMPARISON — All Cycles vs C34 Only                    ║');
    console.log('╚══════════════════════════════════════════════════════════════════════╝\n');

    console.log('=== Strategy 1: All Cycles (10-100) ===');
    await createStratAndBackprop(client, 'Universal 1m - C10-C100 Broad', 10, 100, 5);

    console.log('=== Strategy 2: C34 Only ===');
    await createStratAndBackprop(client, 'Universal 1m - C34 Only', 34, 34, 5);

    console.log('✓ Done. Check /signals → Hedged Pairs for comparison.');
